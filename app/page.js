...
const App = () => {
  ...
  const onDeleteNode = useCallback(() => {
    if (selectedElements.length > 0) {
      const nodeId = selectedElements[0].id;
  ...
  }, [selectedElements, setNodes, setEdges, setSelectedElements]);
    ...
    <div className="flex flex-row min-h-screen lg:flex-row">
      <div className="flex-grow h-screen" ref={reactFlowWrapper}>
        ...
        >
          ...
          <Panel>
            <button
              className=" m-2 bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded transition-all ease-in-out duration-300"
              onClick={onSave}
            >
              save flow
            </button>
            ...
            <div className=" w-[3.5rem] h-4 bg-black fixed bottom-0 right-[17%] z-50">
              {" "}
              {""}
            </div>
          </Panel>
      ...
      <Sidebar
        ...
        onDeleteNode={onDeleteNode} // Pass delete handler to Sidebar
      />
    </div>
  );
};
